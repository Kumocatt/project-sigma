...
game
	var
		...
		*/
		...
		invincible_one		= 0		// randomly selects a makes an enemy temporarily invincible.
	...
	proc
		...
		init_game()
			...
			for(var/mob/player/p in participants)
				...
				animate(p, alpha = 255, time = 10, loop = 1)
	...
	//			p.shield()
		...
		init_wave()
			...
			if(current_round == 5)
				boss_mode = 1
				boss_doppleganger()
			if(!boss_mode && prob(50))	// stackable wave types
				if(prob(25))
	...
	//				p.overlays -= p.spotlight
				for(var/i = 1 to enemies_total)										// enemy spawning.
					...
					if(beholder_only || !crawler_only && current_round >= 3 && prob(15))
