...
proc
	get_projectile(_type, _dir, _damage = -1, _max_range = 5, _accuracy = 1, _kb_dist = 4, _sway = 1)
		if(_type && _dir)
			if(active_game.laser_madness) 	_type = /obj/projectile/laser
			if(active_game.nyan_madness)	_type = /obj/projectile/nyan

			var/obj/projectile/p 	= garbage.Grab(_type)
			p.dir					= _dir
			p.damage				= _damage
			p.max_step				= _max_range
...
obj
	weapon
			...
			use(mob/m)
				...
				if(m.client)
					if(m.dir != m:trigger_dir)
						...
						m.dir = m:trigger_dir
				if(m.client) m:flick_arms("base-club-attack")
				for(var/mob/p in obounds(m,16))
					if(get_dir(m, p) == m.dir)
						if(p.can_hit)
							p.knockback(8, get_dir(m,p))
							p.edit_health(damage, m)
				sleep recharge/2
				m.move_disabled = 0
				sleep recharge/2
				can_use			= 1
		...
		sword
			damage		= -5
			recharge	= 1
			drop_type	= /obj/item/melee/sword
			...
			use(mob/m)
				...
				if(m.client)
					if(m.dir != m:trigger_dir)
						sleep world.tick_lag*2
						m.dir = m:trigger_dir
				if(m.client) m:flick_arms("base-sword-attack")
				for(var/mob/p in obounds(m,24))
					if(get_dir(m, p) == m.dir)
						if(p.can_hit)
							p.knockback(3, get_dir(m,p))
							p.edit_health(damage, m)
				sleep recharge/2
				m.move_disabled = 0
				sleep recharge/2
		...
		gun
			var/tmp
				max_range		= 64 	// how many PIXELS the projectile can travel. round(max_range/step_size) = how many steps the projectile can make.
				accuracy		= 10 	// stray 1px from center every [accuracy] steps. max = 10
