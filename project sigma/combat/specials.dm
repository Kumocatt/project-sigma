...
obj
	weapon
		special
			var/tmp
				max_range	= 4 // how many PIXELS the projectile can travel.
				accuracy	= 6 // stray 1px from center every [accuracy] steps
				sway		= 1
				recoil		= 1
				...
				use(mob/m)
					...
					if(m.client)
						if(m.dir != m:trigger_dir)
							...
							m.dir 				= m:trigger_dir
					var/obj/projectile/p 		= get_projectile(/obj/projectile/grenade, m.dir, damage, round(max_range/6), accuracy, kb_dist, sway)
					if(m.client) m:flick_arms("base-grenade-attack")
					p.loc = m.loc
					switch(m.dir)
						if(NORTH)
							p.step_x	= m.step_x
